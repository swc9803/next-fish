...
const createWeaponLevels = (): Record<WeaponKind, number> => ({
	standard: 1,
	fork: 0,
	scatter: 0,
	lance: 0,
...
export const createInitialState = (highScore: number, metaProgress?: MetaProgress): GameState => {
	...
	state.stageMission = createStageMission(state, state.stage, state.routeModifier);
...
export const makeHud = (state: GameState): HudState => ({
	...
	suppliesCollected: state.suppliesCollected,
	earnedCoins: state.earnedCoins,
	stageMission: state.stageMission,
	routeModifier: state.routeModifier,
	rewardMultiplier: state.rewardMultiplier * state.routeRewardMultiplier,
	threatMultiplier: state.threatMultiplier * state.spawnIntensityMultiplier,
});
