import { useEffect, useMemo, useRef, useCallback, RefObject, Dispatch, SetStateAction, memo } from "react";
	...
	({
		...
		const animateBomb = useCallback(
			(indexes: number[], fish: Object3D) => {
				...
				indexes.forEach((index) => {
					...
					if (cellTweens.current[index]) cellTweens.current[index]?.kill();
					...
					const tween = gsap.to(color, {
						...
						ease: "power1.inOut",
						onComplete: () => {
							if (!isGameOver && checkCollision(fish, index)) handleFishHit(index);
							color.set("white");
			...
			[isGameOver, checkCollision, handleFishHit, meshRefs, cellTweens]
		);
		...
		useEffect(() => {
			...
			const interval = setInterval(() => {
							...
							? [
									{
										...
										position: [(Math.floor(Math.random() * 7) - 3) * CELL_SIZE - 50, 1, (Math.floor(Math.random() * 7) - 3) * CELL_SIZE],
									},
							  ]
							: prev
					);
				...
				if (bombSpawnTimer >= 1500 && fishRef.current) {
					const availableIndexes = [...Array(CELLS.length).keys()].filter((i) => !activeBombsRef.current.has(i));
					const selected = availableIndexes.sort(() => 0.5 - Math.random()).slice(0, 3);
					animateBomb(selected, fishRef.current);
					bombSpawnTimer = 0;
				}
			}, 1000);

			return () => clearInterval(interval);
		}, [bombActive, isInBombZone, isGameOver, animateBomb, setFeeds]);
		...
		useEffect(() => {
			...
			Object.values(cellTweens.current).forEach((tween) => tween?.kill());
			cellTweens.current = {};
			...
			<>
				...
				{feeds.map(({ id, position }) => (
					<GrowingFeed
						...
						onCollected={() => !isGameOver && setFeeds([])}
						onExpire={() => setFeeds([])}
					/>
				))}
			</>
